...
--
-- "Predefined type System.Object is not defined" almost always means:
--   1) server started without DOTNET_ROOT / SDK on PATH
--   2) project not restored (`dotnet restore`)
--   3) broken/missing entry in a .sln
--   4) diagnostics before project finished loading
...
local cmd = vim.fn.executable(roslyn_bin) == 1 and { roslyn_bin, "--stdio" }
  or { "roslyn-language-server", "--stdio" }
